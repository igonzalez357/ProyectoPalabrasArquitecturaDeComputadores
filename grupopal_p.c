...
int main (int argc, char *argv[]) {
	...
	while (ngrupos < MAX_GRUPOS && convergencia_cont < 1) {
		...
		struct vec_grupos vecGrupos[64];
		int tid, nth;

		for (i = 0; i < 64; i++) {
			for (j = 0; j < ngrupos; j++) vecGrupos[i].nvecgs[j] = 0;
		}

		#pragma omp parallel private(i, num, grupo, tid, nth) shared(popul, vecGrupos)
		{
			...
			for (i = 0; i < nvec; i++) {
				grupo = popul[i];
				num = vecGrupos[tid].nvecgs[grupo];
				vecGrupos[tid].vecgs[grupo][num] = i;
				vecGrupos[tid].nvecgs[grupo]++;
			}
		}
			
		for (i = 0; i < ngrupos; i++) {
			for (j = 0; j < 64; j++) {
				num = vecGrupos[j].nvecgs[i];
				for (k = 0; k < num; k++) listag[i].vecg[num] = vecGrupos->vecgs[i][k];
				listag[i].nvecg += num;
			}
		}*/
	...
	printf ("\n>> Numero de clusteres: %d. Tamano de los grupos:\n\n", ngrupos);
	ind = 0;
	for (i = 0; i < ngrupos / 10; i++) {
		for (j = 0; j < 10; j++){
